...
radon_by_row_max = [0, 0, 0]
img = cv2.imread("test.jpeg", 0)
fft = my_fft(img)
img_blur = motion_blur(img, 20, 0)
fft_blur = my_fft(img_blur)
...
for i in range(len(max_loc)):
    ...
    plt.subplot(337+i),plt.plot(R_fft[max_loc][i][0])

myshow(1, R_fft.T, 'Radon Blur FFT')
plt.show()
